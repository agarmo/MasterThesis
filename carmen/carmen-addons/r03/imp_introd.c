...
static void fork_speech(char *s, int question, int answer) {
  ...
  */
  ...
  if (question) {
    if (answer)
      sprintf(wav_filename, "/home/jrod/wavs/IMP1-%dy%d.wav", num, num2++);
    else
      sprintf(wav_filename, "/home/jrod/wavs/IMP1-%dn%d.wav", num, num2++);
  }
  else {
    num2 = 0;
    sprintf(wav_filename, "/home/jrod/wavs/IMP1-%d.wav", num++);
  }
